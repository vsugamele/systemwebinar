...
export function getSessionModeFromPath(pathname, isEvergreen) {
  if (typeof pathname === 'string' && pathname.startsWith('/replay')) return 'replay'
  return isEvergreen ? 'evergreen' : 'live'
}

export function getMetricDelta(current, previous) {
  const currentValue = Number(current) || 0
  const previousValue = Number(previous) || 0
  return {
    absolute: currentValue - previousValue,
    pct: previousValue > 0 ? Math.round(((currentValue - previousValue) / previousValue) * 100) : null,
  }
}
