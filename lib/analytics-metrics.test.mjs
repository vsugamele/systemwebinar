...
test('emits watch samples once per interval-aligned timestamp', () => {
  const sent = new Set()
  assert.equal(shouldEmitWatchSample(5, 5, sent), true)
  assert.equal(shouldEmitWatchSample(5, 5, sent), false)
  assert.equal(shouldEmitWatchSample(6, 5, sent), false)
  assert.equal(shouldEmitWatchSample(10, 10, sent), true)
  assert.equal(shouldEmitWatchSample(10, 10, sent), false)
...
test('classifies webinar room mode from route and evergreen flag', () => {
  assert.equal(getSessionModeFromPath('/replay/my-webinar', false), 'replay')
  assert.equal(getSessionModeFromPath('/w/my-webinar', true), 'evergreen')
  assert.equal(getSessionModeFromPath('/w/my-webinar', false), 'live')
})

test('calculates absolute and percent metric deltas', () => {
  assert.deepEqual(getMetricDelta(120, 100), { absolute: 20, pct: 20 })
  assert.deepEqual(getMetricDelta(80, 100), { absolute: -20, pct: -20 })
  assert.deepEqual(getMetricDelta(12, 0), { absolute: 12, pct: null })
})
